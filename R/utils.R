  ...
  function(res) {
    ...
    error_message <-
      switch(
        ...
        `4700` = c(
          "Zoom API reported an authentication error (4700):",
          "Your access token does not contain permission to access the",
          "requested API endpoint scopes."
        ),
        `4711` = c(
          "Zoom API reported an invalid access token (4711):",
          "Invalid access token, does not contain required scopes."
        ),
        `300` = c(
          "Zoom API reported an invalid request (300):",
          "The next page token is invalid or expired."
        ),
        `400` = c("Zoom API reported an invalid request:", "Bad Request."),
        `3001` = c(
          "Zoom API reported an invalid request (3001):",
          "Meeting/Webinar ID does not exist"
        ),
        `401` = c("Zoom API reported an invalid access token (401)"),
        `404` = c("Zoom API reported webinar Id is not found or has expired."),
        # Default response for unknown status code:
        c(
          glue::glue(
            "Zoom API reported an atypical status code {res$status_code}"
          ),
...
#'
#' @return A named character vector where names are meeting type codes (as strings)
#' and values are human-readable descriptions.
#'
#' @keywords internal

get_meeting_format_labels <- function() {
  c(
    "1" = "Instant meeting",
    "2" = "Scheduled meeting",
